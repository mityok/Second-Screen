package com.cellcom.soccer{
	...
	public class GalleryScreen extends ViewDisplayObject {
		...
		public function GalleryScreen() {
			...
			imageContainerMask.graphics.drawRect(0,0,UiConst.WIN_WIDTH,UiConst.WIN_HEIGHT);
		...
		private function initInitial():void {
			for (var i:int=0; i<UiConst.GALLERY_COLUMNS*UiConst.GALLERY_ROWS; i++) {
				addImage(i);
			}
			redrawScroller();
		}
		private function addImage(count:int):void {
			...
			var yPos:int = count / UiConst.GALLERY_ROWS;
			var xPos:int = count % UiConst.GALLERY_COLUMNS;
			image.x=xPos*(GalleryImage.IMG_WIDTH+UiConst.GALLERY_SPACING);
			image.y=yPos*(GalleryImage.IMG_HEIGHT+UiConst.GALLERY_SPACING);
			images.push(image);
		...
		public function insertImage(data:Object):void {
			if (images.length <= loadedImagesCounter) {
				...
				if (checkRowAddition()&& !inMotion) {
					TweenLite.to(imageContainer,1,{y:-imageContainer.height+UiConst.WIN_HEIGHT});
				}
			}
			redrawScroller();
		...
		private function redrawScroller():void {
			...
			var perc:Number = 0;
			if (imgHgt!=0) {
				perc = UiConst.WIN_HEIGHT / imgHgt;
			}
			scroller.graphics.clear();
			if (perc<1) {
				var scrHgt:Number=UiConst.WIN_HEIGHT*perc;
				
				scroller.graphics.beginFill(UiConst.BG_ORANGE,0.5);
				scroller.graphics.drawRoundRect(2,0,8,scrHgt,4,4);
				scroller.graphics.endFill();
			}
		}
		private function checkRowAddition():Boolean {
			var oldPos:int = (loadedImagesCounter - 1) / UiConst.GALLERY_ROWS;
			var newPos:int = loadedImagesCounter / UiConst.GALLERY_ROWS;
		...
		public function moveY(deltaMotionY:Number) {
			...
			} else if (this.imageContainer.y<-imageContainer.height+UiConst.WIN_HEIGHT) {
				this.imageContainer.y =  -  imageContainer.height + UiConst.WIN_HEIGHT;
			}
			var perc:Number=this.imageContainer.y/(-imageContainer.height+UiConst.WIN_HEIGHT);
			
			scroller.y=(-scroller.height+UiConst.WIN_HEIGHT)*perc;
